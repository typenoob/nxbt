...
install-deps:
	...
	if [ -f /etc/alpine-release ]; then \
		$$SUDO apk update && \
		$$SUDO apk add ccache make gcc g++ python3 python3-dev py3-pip libusb dbus-dev openssl-dev patchelf procps zstd bluez; \
	elif [ -f /etc/debian_version ]; then \
		$$SUDO apt update && $$SUDO apt install -y wget ccache make gcc g++ python3 python3-dev python3-pip libusb-1.0-0 libssl-dev libdbus-1-dev patchelf procps zstd bluez; \
	else \
