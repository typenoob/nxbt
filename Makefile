...
install-deps:
	...
	if [ -f /etc/alpine-release ]; then \
		$$SUDO apk update && \
		$$SUDO apk add ccache make gcc g++ python3 python3-dev libusb dbus-dev patchelf procps bluez; \
	elif [ -f /etc/debian_version ]; then \
		$$SUDO apt update && $$SUDO apt install -y wget ccache make gcc g++ python3 python3-dev libusb-1.0-0 libssl-dev libdbus-1-dev patchelf procps bluez; \
	else \
